...
- **Delegation privacy boundary (internal channel)**: Multi-agent coordination raises total system exposure through inter-agent messages (measured at 68.8% leakage; output-only audits miss 41.7% of violations). Subagent prompts dispatched to non-workspace personas (`researcher`, `synthesizer`, `auditor` for wiki) MUST NOT contain `workspace/` paths, personal identifiers, or profile attributes. Only tasks explicitly scoped to `workspace/` (e.g. `protocol-architect`) may receive workspace context. When `protocol-architect` delegates to `researcher` or `synthesizer` to fill knowledge gaps, it must translate personal constraints into generic, objective research questions before delegating.
...
* **Podarcis Engine (`.podarcis/` & `podarcis` CLI)**: Unified Python CLI and runtime engine for status inspection (`podarcis status`), configuration (`podarcis config`), multi-workspace git/gdrive syncing (`podarcis repo sync`), testing (`podarcis test`, which runs pytest and then the front-end crate's tests), and link linting (`podarcis lint`). Everything under `.podarcis/` is imported as `podarcis.*` and **only** as `podarcis.*` — never add a `sys.path` insert to reach a sibling module, or it becomes a second instance of itself with its own copy of module state. `podarcis lint` exits non-zero on findings; the `commit` and `push` autonomy gates depend on that. Linting itself is Rust — `.podarcis/audit.py` owns the *gate* (what a finding means for a commit) and shells to the `podarcis` binary for the verdict.
* **Frontend (`tui/`, `podarcis build`, bare `podarcis`)**: A single Rust/Ratatui binary (`podarcis-tui`) that owns the terminal: content tree, markdown reader, OKF inspector, vim-modal editor, instant fuzzy and text search, and a live **herdr** pane in the sidebar (a real herdr process on a pty — `[experimental] allow_nested = true` in the generated session config is what permits it). It reads `wiki/`, `workspace/` and `sources/` directly, so navigation and typing never spawn a process — and neither do its actions any more: lint, repo sync, commit, repo config, and semantic search are native Rust, ported from the Python engine (only `qmd` itself, an external binary, is still shelled out to, for semantic search). The linter lives here too, and only here: it began as a port of a `check_links.py` that existed in two identical copies, policed by a `podarcis-tui --lint` differ, and once the port was proven equal over the whole corpus the copies and the differ were deleted. `podarcis lint`, `wiki_lint`, the agent-job autonomy gate and the front-end's own commit gate all reach the same code, so the gate and the gutter cannot disagree. The Python engine therefore needs a built binary to lint: a gate that cannot run its checker reports failure rather than passing quietly. Everything else in `.podarcis/` (install/uninstall, the job scheduler, the MCP gateway, `gdrive_sync.py`) stays Python for now — later phases of the same effort. The house palette is `podarcis` (sunlit limestone) and `podarcis-dark` (basalt) — one palette in two grounds: olive ink, wall-lizard green as the accent, and the CLI banner's cyan kept as the link colour so both front ends read as one product; twenty borrowed flavours (Catppuccin, Nord, Gruvbox, …) remain as alternates, and the active flavour is written into the embedded herdr's config so the sidebar matches. The tree column's collections are sized by hand, not by content: drag the boundary under a box to resize it, click the `▾` on its header rule (or press `-`) to fold it to that rule, and both survive a restart in `tui.collection_heights` / `tui.collapsed_collections`. The last open box absorbs the remainder, which is what keeps the column exactly full at any terminal height without storing a fraction — so its own stored height is never consulted. The tree column's collections are sized by hand, not by content: drag the boundary under a box to resize it, click the `▾` on its header rule (or press `-`) to fold it to that rule, and both survive a restart in `tui.collection_heights` / `tui.collapsed_collections`. The last open box absorbs the remainder, which is what keeps the column exactly full at any terminal height without storing a fraction — so its own stored height is never consulted. The home screen — the one with the mark on it — is a position in the navigation history rather than the absence of one (`app::Spot`), so `←`/`→` and `g h` reach it like any page: back from the first page you open lands there, and it draws the same arrows so you can click your way out again. The mark is drawn in two tones: the block elements are the animal and take the accent, every other glyph is the frame it sits against and is drawn quieter, so the lizard reads first. That split is `brand::is_body`, mirrored in `banner.py`. If the art ever puts a box-drawing stroke *inside* the body again, it needs a third ink rather than that predicate — a hairline tinted with the accent reads as a thread beside a solid block, so such a cell has to be filled with the accent and the glyph carved out of it. The mark and the wordmark live in `.podarcis/logo.txt` and `.podarcis/brand.txt`, which the Python banner reads and the Rust front end `include_str!`s — one declaration each, because a second copy of a drawing drifts the first time someone nudges a pixel. Saving is guarded: the editor stamps the file's mtime when it opens a buffer and refuses to write if it moved, because `wiki/` has more than one writer — an external editor on the same folder, `wiki_publish`, a `git pull` — and the loser of that race would otherwise be whoever saved last, silently. A second ctrl+s overwrites and parks the outside version beside the page as `<name>.conflict-<n>.md`, which is a real page in the collection: it is linted, indexed and committed like any other, so resolve it rather than leaving it. Build it with `podarcis build`; `podarcis status` reports whether it is built. The installer symlinks `podarcis-tui` into `~/.local/bin` alongside `podarcis`, because bare `podarcis` run against a *different* checkout has no `tui/` crate there to build from and finds the binary on `$PATH` or not at all. There is no `wiki` or `frontend` subcommand and no `frontend:` config key: bare `podarcis [path]` always opens *this* front-end — a named subcommand for the same action was a redundant alias, and `wiki` was leftover terminology from before the wiki/workspace/sources split, when the whole checkout was colloquially "the wiki." The key used to choose between `tui`, `vscode`, `obsidian` and `none`, and once the other three stopped being supported it could only be set wrong: a checkout carrying `frontend: obsidian` answered bare `podarcis` by launching Obsidian. A setting with one correct value is not a setting.
* **Agent Control Channel (`podarcis tui`)**: The front-end listens on a unix socket (`$PODARCIS_TUI_SOCK`, exported into the herdr pane so any agent working there inherits it), and `podarcis tui open|highlight|ask` is the client — the same JSON-over-a-socket shape the front-end uses to drive herdr, pointed the other way. It exists so an agent can *show* its work instead of describing it: `podarcis tui open <path> --line N` puts a page on screen, `podarcis tui highlight <path> --lines 40-58 --label "unsourced claim"` tints those source lines in the reader (`g m` clears them), and `podarcis tui ask "Merge these two pages?" --options yes,no,later` blocks until a human answers and prints the answer on stdout. Line numbers are **file** lines, frontmatter included, the way `Read` shows them. Three rules hold it in place: nothing here writes to `wiki/` or `workspace/` — marks are in-memory and `wiki_publish` stays the only write path; nothing steals the terminal mid-keystroke — a request arriving while the editor is open is acknowledged and queued until you leave it; and with no front-end attached, `open` and `highlight` say so and exit 0, so the same agent still runs in a headless job (`ask` exits 2, having no answer to give, and 4 if the question is dismissed).
* **Configuration (`.podarcis/config.yaml`)**: The single configuration file. There is no `state.yaml` — runtime state (jobs, engines, last sync) lives here alongside everything else, because the MCP servers read this file and a parallel one silently stranded every setting written to it. A legacy `state.yaml` is folded in automatically on first read.
