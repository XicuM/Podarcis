...
* **Wiki Front-End (`tui/` & `podarcis wiki`)**: A single Rust/Ratatui binary (`podarcis-tui`) that owns the terminal: content tree, markdown reader, OKF inspector, vim-modal editor, instant fuzzy and text search, and a live **herdr** pane in the sidebar (a real herdr process on a pty — `[experimental] allow_nested = true` in the generated session config is what permits it). It reads `wiki/`, `workspace/` and `sources/` directly, so navigation and typing never spawn a process; the Python engine is called only for actions (`podarcis lint --json`, `podarcis wiki search --json`, `podarcis repo sync`). Its linter is a faithful port of `wiki_check_links.check_file` and is diffed against the engine with `podarcis-tui --lint` — `podarcis lint` remains the authority and the commit gate, so the two must never disagree. Themes are Catppuccin, latte by default, matching the PodarcisNest containers; the active flavour is written into the embedded herdr's config so the sidebar matches. Build it with `podarcis wiki build`; `podarcis status` reports whether it is built.
