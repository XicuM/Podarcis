...
3. **Protocol Architect (`@protocol-architect`)**: Reads `workspace/profile.md`, adapts Wiki findings into step-by-step personalized protocols in `workspace/protocols/` (loading the external `menumaker` skill for nutritional protocols).
...
├── apm.yml                  # Third-party agent context consumed via APM
├── .claude/, .opencode/     # Harness deploy roots — generated by `apm install`, gitignored
...
* **Model Context Protocol (MCP)**: Native servers (`wiki-mcp`, `research-mcp`, `diagnostics-mcp`) enable knowledge base queries, literature search, and pain-point logging. The bound surface is exactly what a Bash-less agent job can reach; everything a human drives — repo syncing, menu optimization, resolving pain points — is a `podarcis` subcommand instead.
