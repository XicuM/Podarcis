'''Harness-specific MCP config adapters.

Design principle:
  - .agents/mcp_config.json stores ONLY server definitions (command, args, env)
  - Each harness's native config is the source of truth for enable/disable state
  - Podarcis only writes to the ACTIVE harness's config
  - Switching harnesses never touches other harnesses' configs
...
class BaseAdapter(ABC):
    '''Abstract base interface for harness-specific MCP config adapters.'''
...
def get_adapter(harness: str) -> BaseAdapter | None:
    '''Retrieve adapter instance for harness name.'''
    return ADAPTERS.get(harness)


def read_enabled(root: Path, harness: str) -> dict[str, bool]:
    '''Read enable/disable state from specified harness's native config.'''
    if adapter := ADAPTERS.get(harness):
        return adapter.read_enabled(root)
    return {}


def write_enabled(root: Path, harness: str, key: str, enabled: bool) -> None:
    '''Write enable/disable state to specified harness's native config.'''
    if adapter := ADAPTERS.get(harness):
        adapter.write_enabled(root, key, enabled)


def generate_for_harness(root: Path, harness: str) -> Path | None:
    '''Regenerate native config for specified harness.'''
    if adapter := ADAPTERS.get(harness):
...
generate_for_backend = generate_for_harness


def generate_all(root: Path) -> dict[str, Path | None]:
    '''Regenerate native config for all harnesses.'''
    return {name: adapter.generate(root) for name, adapter in ADAPTERS.items()}


sync_all_harnesses = generate_all
sync_all_backends = generate_all
