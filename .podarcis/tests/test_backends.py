'''Unit tests for backend MCP configuration adapters.'''
...
def test_adapter_registration():
    '''Verify all expected backends are registered in ADAPTERS dictionary.'''
...
def test_global_helper_functions(tmp_path):
    ...
    # Non-existent backend returns empty dict / None
    assert read_enabled(tmp_path, 'invalid_backend') == {}
    assert generate_for_backend(tmp_path, 'invalid_backend') is None
